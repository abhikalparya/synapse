"""Evaluation adapter for domain_curriculum_prior (opt-in)."""
...
async def run_domain_curriculum_prior(
    example: EvalExample,
    *,
    ...
    try:
        ...
        graph = GeneratedGraph(
            ...
            generation_meta={"generation_strategy": "domain_curriculum_prior"},
        )
        return graph, {
            "llm_latency_ms": 0.0,
            "error": str(exc),
    ...
    try:
        result = await run_domain_curriculum_prior_pipeline(
        ...
        graph = GeneratedGraph(
            ...
            generation_meta={"generation_strategy": "domain_curriculum_prior", "domain": resolved},
        )
        return graph, {
            "llm_latency_ms": 0.0,
            "error": str(exc),
    ...
    if not result.parse_ok or not result.topics:
        graph = GeneratedGraph(
            ...
            dependencies=[],
            parse_ok=False,
            error="; ".join(result.errors) or "domain curriculum prior failed",
            error_category="parse_error",
            generation_meta=meta,
            raw_response=result.selection_raw or result.dependency_raw,
        )
        return graph, meta
    ...
    graph = GeneratedGraph(
        ...
        raw_response=result.dependency_raw or result.selection_raw,
        parse_ok=True,
        generation_meta=meta,
    )
    return graph, meta
